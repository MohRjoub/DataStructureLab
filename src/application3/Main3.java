...
public class Main3 {
		...
		private Button removeDuplicated = new Button("Remove Duplicated");
		private VBox finalBox = new VBox(orginalList, addBox, deleteBox, findBox, recReverseHBox, itrReverseHBox,removeDuplicated);
		...
		public Main3() {
			...
			deleteBtn.setOnAction(e -> {
				try {
					...
					DNode<Integer> found = linkedList.find(Integer.parseInt(data));
					if (found!=null) {
			...
			findBtn.setOnAction(e -> {
				try {
					...
					findRes.setText(linkedList.find(Integer.parseInt(data))+"");
				} catch (Exception e2) {
			...
			removeDuplicated.setOnAction(e->{
				try {
					linkedList.removeDuplecateRecursion();
					orginalList.setText(linkedList.toString());
				} catch (Exception e2) {
				}
			});
